...
#define SWAP_FILE_NAME ("SWAP.SWAP")
...
initSwapOps ()
{
    ...
    int returnErr = vfs_open(swap_file_name, O_CREAT, &swapFile);
...
int
swapIn (int segType, int segOffset, struct addrspace* addrSpace)
{
...
int
swapOut (int segType, int segOffset, struct addrspace* addrSpace)
{
    assert(addrSpace != NULL);
    struct uio copyUIO; 
    int inIns = 0, result;
